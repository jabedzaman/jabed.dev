...
export function getWritings(): MDXFileData[] {
  return getMDXData(path.join(process.cwd(), "content", "writings"));
}

export function getWritingBySlug(slug: string): MDXFileData | null {
  return getWritings().find((writing) => writing.slug === slug) ?? null;
}
