  ...
  return (
    <React.Fragment>
      <motion.nav
        ...
        className={cn(
          "top-40 right-auto left-8 hidden",
          "lg:hidden",
          "xl:top-24 xl:right-24 xl:left-auto xl:block",
          "fixed mt-10 h-full w-48 justify-start space-y-4 transition"
        )}
      >
          ...
          {headings.map((heading) => (
            <div key={heading.id} className="mt-0">
              <button
                ...
                className={cn({
                  "mt-0 ml-2 border-l border-l-border py-1 text-left text-sm text-muted-foreground/30 opacity-100 transition ease-in-out hover:opacity-80":
                    true,
