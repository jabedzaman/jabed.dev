...
export function getCrafts(): MDXFileData[] {
  return getMDXData(path.join(process.cwd(), "content", "crafts"));
}

export function getCraftBySlug(slug: string): MDXFileData | null {
  return getCrafts().find((craft) => craft.slug === slug) ?? null;
}
