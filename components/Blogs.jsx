          ...
          return (
            <div key={key} className="py-8 flex flex-wrap md:flex-nowrap">
              ...
              <div className="md:flex-grow">
                <h2 className="text-2xl font-medium dark:text-gray-200 text-gray-900 title-font mb-2">
                  {blog.data.title}
                </h2>

                <p className="text right-0 lg:text-left text-center text-gray-700 font-light mb-2 flex flex-row space-x-3 items-center">
                    <SiReadthedocs className="mr-2"/>
                    {blog.data.read} mins read
                </p>
                <p className="leading-relaxed text-gray-700 dark:text-gray-300">{blog.data.description}</p>
                <Link
                  passHref
                  href={`blogs/${blog.filePath.replace(/\.mdx?$/, "")}`}
                >
                  <p className="text-indigo-500 cursor-pointer inline-flex items-center mt-4">
                    Read More
                    <svg
